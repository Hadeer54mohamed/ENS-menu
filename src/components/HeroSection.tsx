...
const HeroSection = () => {
  ...
  return (
    <section id="hero" className="pt-32 pb-16 md:pt-40 md:pb-24 bg-gradient-hero overflow-hidden relative">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute bottom-20 left-10 w-96 h-96 bg-primary/5 rounded-full blur-3xl animate-pulse-slow" style={{ animationDelay: "1s" }} />
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className={`flex flex-col ${isRTL ? 'md:flex-row-reverse' : 'md:flex-row'} items-center gap-12 md:gap-16`}>
          {/* Text Content */}
          <div className={`flex-1 text-center ${isRTL ? 'md:text-right' : 'md:text-left'}`}>
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold leading-tight mb-6 animate-slide-up" suppressHydrationWarning>
              {t("hero.title1")}
              <br />
              <span className="text-gradient animate-gradient">{t("hero.title2")}</span>
            </h1>
            <p className="sr-only">
  منيو إلكتروني QR Code للمطاعم والكافيهات في مصر، طلب ودفع بدون عمولات
</p>

            <p className="text-lg md:text-xl text-muted-foreground max-w-2xl mx-auto md:mx-0 mb-8 animate-slide-up opacity-0" style={{ animationDelay: "0.2s", animationFillMode: "forwards" }} suppressHydrationWarning>
              {t("hero.description")}
            </p>
            <div className="animate-slide-up opacity-0" style={{ animationDelay: "0.4s", animationFillMode: "forwards" }}>
              <Button variant="hero" size="xl" className="group hover:scale-105 transition-all duration-300 shadow-glow" asChild suppressHydrationWarning>
                <a href="#contact">
                  {t("hero.cta")}
          ...
          <div className="flex-1 flex justify-center perspective-1000">
            <div className="relative animate-float">
             ...
             {/*  <img
              ...
              <div className="absolute -bottom-4 -left-4 w-12 h-12 bg-primary/30 rounded-full blur-lg animate-float" style={{ animationDelay: "0.5s" }} />
            </div>
          </div>
        </div>
      </div>
